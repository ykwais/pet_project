...
public class ServiceFilteringMain {
    ...
    private static void initMetrics(Config config) {
        try {
            ...
            //Gauge.builder("process_files_open", () -> 0).register(registry);
...
//            new JvmThreadMetrics().bindTo(registry);
//
//            // запускаем самописный HTTP-сервер
//            startMetricsServer();
//
//            Runtime.getRuntime().addShutdownHook(new Thread(() -> log.info("Metrics server shutting down")));
//
//            log.debug("Sample metrics:\n{}", registry.scrape());
//        } catch (IOException e) {
//            log.error("Failed to start metrics server", e);
//        }
//    }
...
//            Gauge.builder("system_cpu_count", osBean,
//                            OperatingSystemMXBean::getAvailableProcessors)
//                    .register(registry);
//            Gauge.builder("system_cpu_usage", osBean,
//                            OperatingSystemMXBean::getSystemLoadAverage)
//                    .register(registry);
//
//            // JVM метрики (необходимы для дашборда)
//            new JvmMemoryMetrics().bindTo(registry);
//            new JvmGcMetrics().bindTo(registry);
//            new ClassLoaderMetrics().bindTo(registry);
//            new JvmThreadMetrics().bindTo(registry);
